...
//
// 채팅으로 하나씩 묻는 대신, 수강생이 이 파일에 다섯 가지를 적고 "다 넣었어요" 라고 하면 코덱스가 읽어 확인·저장한다 (2026-09-20 진현님 지시).
const 키설정파일 = (작업폴더) => join(승인글폴더(작업폴더), "00_설정", "키설정.txt");
const 키설정템플릿 = `# 메킷 키트 키 설정 — 아래 = 뒤에 값을 붙여넣고 저장(Ctrl+S / Cmd+S)한 뒤, 코덱스에게 "다 넣었어요" 라고 말하세요.
...
export async function 키설정적용({작업폴더, 내용 = "", 수강코드목록 = 기본수강코드}) {
  ...
  // 충전액 (선택)
  if (값.충전한달러) {
    const r = await 충전액저장({작업폴더, 달러: 값.충전한달러});
    결과.충전액 = r.저장 ? `${r.충전달러}달러` : "숫자가 아님";
  } else 결과.충전액 = "안 넣음 (선택)";
  // 사이트 1~10
  for (let n = 1; n <= 10; n += 1) {
    ...
    if (!주소 && !아이디 && !비번) {
      if (n === 1) 결과.사이트.push({번호: 1, 상태: "비어 있음"});
      continue;
    }
    if (!주소 || !아이디 || !비번) {
      결과.사이트.push({번호: n, 상태: "세 칸 중 빈 칸이 있음"});
      continue;
    }
  ...
  결과.사이트수 = 결과.사이트.filter((s) => s.상태 === "연결 됨").length;
  결과.사이트목록 = 결과.사이트.filter((s) => s.상태 === "연결 됨").map((s) => `${s.번호}번 ${s.주소}`);
  결과.파일 = 파일;
  return 결과;
}
