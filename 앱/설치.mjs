...
`, "utf8");
  ...
  return {
    ...
    다음: 전부됨 ? "설치 끝. 수강생에게 딱 이렇게만 말하고 답을 기다린다: '설치가 끝났어요. 이제 처음 한 번만 하는 키 설정입니다. 공지에서 받은 키설정.txt 파일을 여기에 끌어다 놓거나, 그 내용을 그대로 붙여넣어 주세요.' 항목을 하나씩 묻지 않는다. 수강생이 파일이나 내용을 주면 작업 폴더의 AGENTS.md 대본대로 준비 코드를 실행하고 키설정적용을 부른다. 새 채팅을 열라고 하지 않는다." : "",
  };
}
