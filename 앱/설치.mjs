...
export async function 설치({작업폴더}) {
  ...
  return {
    ...
    다음: 전부됨 ? "설치 끝. 수강생에게 딱 두 문장만 한 번에 말하고 답을 기다린다: '설치가 끝났어요. 이제 처음 한 번만 하는 키 설정을 할게요.' 그리고 '강의 자료실 공지에 있는 수강 코드를 알려 주세요.' 답을 받으면 작업 폴더의 AGENTS.md 대본대로 준비 코드를 실행하고 수강 코드를 저장한다. 새 채팅을 열라고 하지 않는다." : "",
  };
}
