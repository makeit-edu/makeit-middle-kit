...
export async function 설치(뿌리 = path.resolve(".")) {
  ...
  try {
    ...
    const 전역폴더 = path.join(homedir(), ".codex");
    const 전역파일 = path.join(전역폴더, "AGENTS.md");
    const 대본 = await readFile(path.join(뿌리, "AGENTS.md"), "utf8");
    const 시작표 = "<!-- 네이버글도우미 시작 (설치.mjs 가 관리. 손으로 고치지 마세요) -->";
    const 끝표 = "<!-- 네이버글도우미 끝 -->";
    let 기존 = "";
    ...
    let 새것;
    if (a >= 0 && b > a) 새것 = 기존.slice(0, a) + 덩어리 + 기존.slice(b + 끝표.length);
    else 새것 = (기존.trim() ? 기존.trimEnd() + "\n\n" : "") + 덩어리 + "\n";
    ...
    기록["~/.codex/AGENTS.md (전역 대본)"] = "실패: " + String(e?.message || e).slice(0, 60);
  }
  const 전부됨 = Object.values(기록).every((v) => v === "받음" || v === "갱신");
