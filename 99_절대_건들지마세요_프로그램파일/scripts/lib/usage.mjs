...
// 글 하나가 끝났을 때 찍는 두 줄. 항상 찍는다 — 예산이 없으면 그 자리에 "충전액을 넣으면 보여 드려요"를 찍는다.
export function 돈줄({이글, 이번실행, 예산}) {
  ...
  const 둘째 = 예산.있음
    ...
    : `     이번 실행 ${이번실행.글수}개 · 약 ${천단위(이번실행.krw)}원 │ 전체 누적 ${예산.전체.글수}개 · 약 ${천단위(예산.전체.krw)}원 │ 남은 돈: '키설정'에서 충전한 금액을 넣으면 보여 드려요`;
  return [첫줄, 둘째];
}
